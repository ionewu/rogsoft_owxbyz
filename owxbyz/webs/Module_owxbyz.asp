...
<head>
    ...
    <title>软件中心 - 驿站（yz.iepose.com）</title>
...
var PROTOCOL = "cifs";
        ...
        var params_check = ["owxbyz_enable"];
        var params_input = ["owxbyz_cachepath"];
        var dbus = {}
	...
	require(['/require/modules/diskList.js'], function(diskList) {
		...
		for (var i = 0; i < usbDevicesList.length; ++i){
			for (var j = 0; j < usbDevicesList[i].partition.length; ++j){
				//append options
                var partName = usbDevicesList[i].partition[j].partName;
                ...
                var options = "<option value='/mnt/" + partName;
                    if (dbus["owxbyz_cachepath"] == "/mnt/" + partName) {
                        options += "' selected>";
                        selected = true;
                    } else {
                        options += "'>";
                    ...
                    options += partName;
                    options += ", " + usedsize + "/" + totalsize + " GB";
                    options += "</option>";
                console.log(options);
				$("#owxbyz_cachepath").append(options);
			}
		}
        if(!selected) {
            $("#owxbyz_cachepath").append("<option value='' selected></option>");
        }
	});
}
        ...
        function push_data(obj, arg) {
            ...
            var postData = { "id": id, "method": "owxbyz_config.sh", "params": [arg], "fields": obj };
            $.ajax({
                url: "/_api/",
                cache: false,
                type: "POST",
        ...
        function version_show() {
            $("#owxbyz_showver").html(dbus["owxbyz_version"]);
        }
        function get_status() {
            var id = parseInt(Math.random() * 100000000);
            var postData = { "id": id, "method": "owxbyz_status.sh", "params": [1], "fields": "" };
            $.ajax({
                type: "POST",
                cache: false,
                url: "/_api/",
        ...
        function menu_hook(title, tab) {
            tabtitle[tabtitle.length - 1] = new Array("", "驿站 - 边缘计算");
            tablink[tablink.length - 1] = new Array("", "Module_owxbyz.asp");
        }
        ...
        function check_dir_path() {
            var dir_array = E('owxbyz_dir').value.split("/");
            if (dir_array[dir_array.length - 1].length > 21)
                E('owxbyz_dir').value = "/" + dir_array[1] + "/" + dir_array[2] + "/" + dir_array[dir_array.length - 1].substring(0, 18) + "...";
        }
        ...
        function confirm_folderTree() {
            E('owxbyz_dir').value = path_directory;
            this.FromObject = "0";
            $("#folderTree_panel").fadeOut(300);
        }
...
<body onload="init();">
    ...
    <input type="hidden" name="current_page" value="Module_owxbyz.asp">
    <input type="hidden" name="next_page" value="Module_owxbyz.asp">
                            ...
                            <table width="760px" border="0" cellpadding="5" cellspacing="0" bordercolor="#6b8fa3"
                                ...
                                <tr>
                                    <td bgcolor="#4D595D" colspan="3" valign="top">
                                        <div>&nbsp;</div>
                                        <div class="formfonttitle">驿站 - 边缘计算，躺赚收益</div>
                                        ...
                                        <div class="SimpleNote">
                                            <li><i>驿站简介: </i>租用闲置设备资源，躺赚积分收益。影视VIP、音乐VIP、话费随心兑换。【玩物科技出品】</li>
                                            <li><i>官方网站: </i><a href="https://yz.iepose.com/" target="_blank"><em><u>https://yz.iepose.com/</u></em></a></li>
                                            <li><i>当前版本: </i><label id="owxbyz_showver"></label></li>
                                        </div>
                                        <table width="100%" border="1" align="center" cellpadding="4" cellspacing="0"
                                            bordercolor="#6b8fa3" class="FormTable">
                                            <thead>
                                                <tr>
                                                    <td colspan="2">驿站 - 高级设置</td>
                                                </tr>
                                            </thead>
                                            <tr id="switch_tr">
                                                <th>
                                                ...
                                                <td colspan="2">
                                                    <div class="switch_field" style="display:table-cell;float: left;">
                                                        <label for="owxbyz_enable">
                                                            <input id="owxbyz_enable" class="switch" type="checkbox"
                                                                style="display: none;">
											...
											<tr id="owxbyz_select">
												...
												<td>
 													<select name="owxbyz_cachepath" id="owxbyz_cachepath"  class="input_option" ></select>
												</td>										
											</tr>
                                        </table>
                                        <div class="owxbyz_msg">
                                            <i>使用流程：</i>
                                            <div class="owxbyz_msg_info">
                                                <li>启动插件：打开上方开关按钮后点击提交。</li>
												<li>先插入移动硬盘，并推荐格式化为 EXT3/EXT4 的文件分区</li>
                                                <li>绑定设备：</li>
												  <ul>一：点击上方"打开驿站"按钮，进入驿站的管理页面。</ul>
												  <ul>二：微信扫码登陆后，设备即自动绑定。</ul>
                                                <li>至此，您可躺赚积分收益。影视VIP、音乐VIP、话费随心兑换。</li>
                                            </div>
                                        </div>
