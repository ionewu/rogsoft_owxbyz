...
install_now(){
	# default value
	local TITLE="驿站"
	local DESCR="驿站 - 租用闲置设备资源，躺赚积分收益。影视VIP、音乐VIP、话费随心兑换。【玩物科技出品】"
	local PLVER=$(cat ${DIR}/version)

	# stop first
	local ENABLE=$(dbus get ${module}_enable)
	if [ "${ENABLE}" == "1" -o -n "$(pidof owxbyz)" ];then
		echo_date "安装前先关闭${TITLE}插件，以保证更新成功！"
		killall	owxbyz > /dev/null 2>&1
        /koolshare/bin/owxbyz/stop.sh > /dev/null 2>&1
	fi

	# remove some file first
	find /koolshare/init.d -name "*owxbyz.sh" | xargs rm -rf >/dev/null 2>&1
	...
	chmod 755 /koolshare/bin/owxbyz/bin/owxbyz >/dev/null 2>&1
